    ...
    pub fn generate(&self, sniff_results: &SniffResults, target_path: &PathBuf) {
        if !target_path.exists() {
            if let Err(e) = fs::create_dir_all(target_path) { 
                println!("Target path ({})  could not be created: {e}", target_path.to_str().unwrap_or_default());
                return;
            };
        }
        ...
        #[cfg(debug_assertions)]
        {
            ...
            println!("Found source root folder: {}", src_root.to_str().unwrap_or_default())
        }
...
<hibernate-configuration>
    ...
    fn generate_tables_files(
        ...
        for table in tables {
            ...
            let table_file_path = target_path.join(format!("{}.hbm.xml", to_upper_camel_case(table.name())));
...
fn to_upper_camel_case(s: &str) -> String {
    capitalize(&to_lower_camel_case(s)).to_string()
}
...
fn get_java_src_root(path: &PathBuf) -> Option<PathBuf> {
    Some(PathBuf::from(env::var("TEST_DIR").unwrap()).join("src"))
}
...
mod test {
    ...
    async fn test_generate_table_xml_trivial() {
        ...
        let generated = generator.generate_table_xml(&sniff_results.database().tables()[0], "com.example");
        let expected = r#"
...
<hibernate-mapping>
    <class name="com.example.Users" table="users">
        <!-- Id -->
