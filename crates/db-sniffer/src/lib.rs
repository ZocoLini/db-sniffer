    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        regex.captures(s).map(|captures| {
            ConnectionParams {
                ...
                password: captures.name("password").map(|pass| pass.as_str().to_string()),
                host: captures.name("host").map(|host| host.as_str().to_string()),
                port: captures.name("port").map(|port| port.as_str().parse().expect("Isn't a number")),
                dbname: captures.name("dbname").map(|dbname| dbname.as_str().to_string()),
            }
...
mod tests
{
    ...
    fn test_connection_params_from_valid_str()
    {
        ...
        assert_eq!(conn_params.user, None); assert_eq!(conn_params.password, None);
        assert_eq!(conn_params.host, None); assert_eq!(conn_params.port, None); 
        assert_eq!(conn_params.dbname, Some("dbname".to_string()));
        ...
        let conn_params = conn_str.parse::<ConnectionParams>().unwrap();
        assert_eq!(conn_params.db, "sqlite");
        assert_eq!(conn_params.user.clone().unwrap(), "user"); assert_eq!(conn_params.password.clone().unwrap(), "password");
        assert_eq!(conn_params.host, None); assert_eq!(conn_params.port, None);
        assert_eq!(conn_params.dbname, Some("dbname".to_string()));
